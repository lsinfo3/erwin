...
$(document).on('click', '.AddTab', function() {
    ...
    // Tab in den Edito hinzuf�gen
    $("div#tabs ul").append('<li><a href="#tabs-' + num_tabs + '">Step_' + num_tabs + '</a></li>');
...
// Funktionen f�r das ConfigPopUp, .custom �ffnen, .closeConfig schlie�en .removeelement element l�schen
$(document).on('click', '.custom', function() {
    var l = $(this).offset().left;
    var t = $(this).offset().top + $(this).height() + 5;

    //todo: hier die Paramter rauslesen und dann direkt beim �ffnen wieder mit anzeigen!
    //$(this).attr('onclick');
    //Test f�r %
    ...
    $('#ElementConfig')
        .html('<div class="col100"><div id="ElementConfigItemName" class="col50"></div><button class="RemoveElement">remove</button><button class="CloseConfig">x</button></div>' +
              '<div class="Clear"></div>' +
              '<div>' +
                  '<div class="col100 ConfigHeaderLine">Functions</div>' +
              '</div>' +
              '<div class="Clear"></div>' +
              '<div>' +
              ...
              '<div id="Styles">' +
                  ...
                  '<label class="col50">Invisible</label><input type="checkbox" id="InvisItem" onchange="CheckForInvis();"/><div class="Clear"></div>' +
                  '<label class="col50">Invisible border</label><input type="checkbox" id="InvisItemBorder" onchange="CheckForInvisBorder();"/><div class="Clear"></div>' +
                  '<label class="col50">Loadanimation</label><input type="checkbox" id="LoadAnim" /><div class="Clear"></div>' +
              '</div>'+
              '<div class="Clear"></div>');
...
function ShowParametersForDelay(val) {
    ...
    $('.tab').each(function() {
        ...
        relTo += '<option value="tabs-' + tc + '"' + selected + '>Step-' + tc + '</option>';
    });
    relTo += '</select>';
    ...
    var paramall = '<div class="col100 ConfigHeaderLine">Parameters</div><div class="Clear"></div>' +
                   '<label class="col50">Delay</label><input type="text" id="DelayTime" class="col40" placeholder="0.0"/><div class="Clear"></div>' +
                   '<label class="col50">Output</label><input type="text" id="ReturnValue" class="col40" placeholder="sample output" /><div class="Clear"></div>' +
                   '<label class="col50">Output to</label>' + outTo + '<div class="Clear"></div>' +
                   '<label class="col50">Link to</label>' + relTo + '<div class="Clear"></div>' +
                   '<label class="col50">Specific URL</label><input type="text" id="CustomUrl" class="col40" placeholder="test.php" /><div class="Clear"></div>'+
                   '<label class="col50">Url modification</label><input type="text" id="ModUrl" class="col40" placeholder="www.test.com" /><div class="Clear"></div>';

    switch (val) {
        case "0":
            $('#Parameter').html('');
            break;
        case "1": // hier nur spezifisches hinzuf�gen
            $('#Parameter').html(paramall +
                                 '<button onclick="addFunction(1)">set function</button>');
            break;
        case "2":
            $('#Parameter').html(paramall +
                                 '<label class="col50">Fixed position</label>' +
                                     '<div class="col40">' +
                                         ...
                                         '</div>' +
                                     '</div>' +
                                 '<button onclick="addFunction(2)">set function</button>');
            break;
        case "3":
            $('#Parameter').html(paramall +
                                 '<label class="col50">Target keyword</label><input type="text" id="Keyword" class="col40" placeholder="sample keyword" /><div class="Clear"></div>' +
                                 '<button onclick="addFunction(3)">set function</button>');
            break;
    }
}
    ...
    switch (type) {
        ...
        case 3:
            ...
            if($('#Script-' + $('#ActElement').text()).length) {
                ...
                $('#HiddenScript').append("<div id='Script-" + $('#ActElement').text() + "' class='dyn_script'>$(document).on('input', '#" + $('#ActElement').text() + "', function() { LoadNextSiteOnChange(" + params + ", '" + $('#ActElement').text() + "', '" + target + "');});</div>");//einf�gen
            }
            break;
    }
    $('#ElementConfig').hide();
...
// Funktion f�r das InstructionsConfigPopUp, .EditInstructions �ffnen, .CloseInstructionsConfig schlie�en
...
$(document).on('click', '.TestActTab', function() {
    //window.history.replaceState('https://localhost/www.mud.de/index7.php', "", "www.amazon.de/ergneiuhrgip�urg");
});
// Testfunktionen  -------------------------------------------------------------------------