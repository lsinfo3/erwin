...
$(document).on('click', '.SaveTabButton', function() {
    ...
    $('.tab').each(function() {
        ...
        if ($('#' + getSelectedTabId() + '-bg').length) { // image bg
            ...
            // größe 100 % falls keine instructions
            if($('input[name="ShowInTabs"]:checked').val() == 0) {
                $('#' + getSelectedTabId() + '-html-bg').css({ width:'100%', height:'100%' });
            } else {
                $('#' + getSelectedTabId() + '-html-bg').css({ width: '80%', height: '100%'});
            }

            // da ifram hintergrund, script hinzufügen das die ausführung von klicks im jeweiligen iframe blockiert.
            dyn_sc_content += ""+
